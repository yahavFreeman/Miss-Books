...
var gBooks = utilService.loadFromStorage(BOOK_KEY);
...
function nextBookById(bookId){
  return query()
    .then(books=>{
      const indx=books.findIndex(book=>book.id===bookId)
      return (indx===books.length-1)? books[0] : books[indx+1].id
    })
}

function previousBookById(bookId){
  return query()
    .then(books=>{
      const indx=books.findIndex(book=>book.id===bookId)
      return (indx===0)? books[books.length-1] : books[indx-1].id
    })
}

function addBook(bookToAdd){
  gBooks.push(bookToAdd)
  _saveBooks()
  console.log(gBooks);
}

function getBooks(searchBy) {
  var searches = utilService.loadFromStorage(SEARCH_KEY) || {};
  if (searches && searches[searchBy])
    return Promise.resolve(searches[searchBy]);
  return axios
    .get(
      `https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchBy}`
    )
    .then((res) => {
      searches[searchBy] = res.data.items;
      console.log("searches server", searches);

      saveSearch(SEARCH_KEY, searches);
      return searches[searchBy];
    });
}
